...
related: [scaffolding, the-agent-loop, grounding, bdi-model]
status: published
...
<Diagram type="mermaid" caption="context_window_constraint // ~128K tokens">
{`graph TB
...
- Code libraries
- Learned heuristics
- Plan libraries (as in BDI architectures)
...
<Callout type="note">
This taxonomy mirrors human memory research. The parallel isn't coincidental—agents face similar memory challenges to biological minds, even if the implementations differ. Classical BDI architectures explicitly separate these memory types into belief bases (semantic), plan libraries (procedural), and intention stacks (episodic active goals).
</Callout>

## Memory Architecture

<Diagram type="mermaid" caption="layered_memory_architecture">
{`graph TD
...
- [BDI Model](/entries/bdi-model) — formal architecture for beliefs, desires, and intentions as memory structures
